<?php

declare(strict_types=1);

return [
    'layout' => 'mare',
    'cidade_nome' => 'Poços de Caldas',
    'geo' => ['lat' => -21.7879, 'lng' => -46.5614],
    'imagem' => 'sudeste.jpg',
    'schema_service_name' => 'Transporte executivo com motorista em Poços de Caldas',
    'seo' => [
        'title' => 'Traslado em Poços de Caldas: Centro, termas e POO',
        'description' => 'Motorista executivo em Poços de Caldas para o Centro, Country Club, termas e o aeroporto POO. Embarque no hotel, espera combinada e orçamento sob consulta.',
        'og_title' => 'Carro com motorista no Centro, no Country Club e no aeroporto POO',
        'og_description' => 'Traslado no sul de Minas: hotel, pauta no Centro e o terminal Walther Moreira Salles.',
        'og_image_alt' => 'Sedan executivo em avenida de Poços de Caldas',
    ],
    'kicker' => 'Sul de Minas · MG',
    'h1' => 'Do POO ao Country Club sem perder a reunião no Centro',
    'editorial' => [
        'h2' => 'Poços se lê em termas e em serra — o app não lê o check-out do Palace',
        'texto' => [
            'Hotel no Centro ou no Country Club, reunião no município e voo no Embaixador Walther Moreira Salles (POO) cabem no mesmo caderno se o motorista já saiu com a ordem da Avenida João Pinheiro, não com o atalho do momento.',
            'O serviço é de traslado executivo: discreto, com espera combinada. O Palace e os hotéis-termas entram como ponto de embarque — nome na portaria, sem parceria ostensiva. Não é corrida de parque das águas no feriado, a menos que a visita às termas esteja no roteiro.',
        ],
    ],
    'coluna_a' => [
        'h2' => 'Três recortes que a pauta pocense usa',
        'itens' => [
            ['h3' => 'Centro e a João Pinheiro', 'texto' => 'Sede e o calçadão. Informe o prédio; o sentido da via muda no fim da tarde.'],
            ['h3' => 'Country Club e o Jardim Country Club', 'texto' => 'Pernoite e o recorte residencial. Informe o nome; “no Country” não define a entrada do condomínio.'],
            ['h3' => 'São Benedito e as termas', 'texto' => 'Hotel e o eixo das águas. O recuo pede o estabelecimento; o Palace e similares são embarque, não parceria.'],
        ],
    ],
    'coluna_b' => [
        'h2' => 'Walther Moreira Salles — POO',
        'texto' => 'O aeroporto fica no recorte da cidade. Combinamos o voo. A espera no desembarque integra o trecho POO. Até o Country Club ou o Centro, a avenida pesa mais que o mapa admite no domingo de saída. Ribeirão Preto e Campinas entram como viagem, não como corrida urbana.',
    ],
    'faq_h2' => 'Dúvidas de quem pousa no Walther Moreira Salles',
    'faq' => [
        ['pergunta' => 'O aeroporto de Poços fica longe do Centro?', 'resposta' => 'É trecho urbano. Marcamos margem: a João Pinheiro e o horário de check-out alteram o intervalo.'],
        ['pergunta' => 'O motorista espera no POO se o voo atrasar?', 'resposta' => 'Sim. O número do voo entra no roteiro e a espera no terminal faz parte do trecho.'],
        ['pergunta' => 'Vocês buscam no Palace e levam ao Country Club?', 'resposta' => 'Sim. Informe o nome do hotel e o recuo da portaria. O endereço entra como embarque, sem parceria.'],
        ['pergunta' => 'Dá para incluir Ribeirão Preto ou Campinas no mesmo dia?', 'resposta' => 'Sim, como viagem. Não é o mesmo recorte do Centro; o horário sai sob consulta.'],
        ['pergunta' => 'São Benedito e o Jardim Country Club cabem no mesmo roteiro?', 'resposta' => 'Sim. Combinamos a ordem. “Nas termas” não define qual dos dois recuos.'],
    ],
    'cta' => [
        'titulo' => 'Anote o hotel junto com o horário do POO',
        'texto' => 'O orçamento de Poços de Caldas sai sob consulta. Termas, Centro e terminal no mesmo recado.',
        'botao' => 'Pedir motorista em Poços de Caldas',
    ],
    'relacionados' => [
        ['tipo' => 'estado', 'slug' => 'minas-gerais', 'ancora' => 'Confins, Vale do Aço e a agenda mineira'],
        ['tipo' => 'cidade', 'estado' => 'sao-paulo', 'cidade' => 'ribeirao-preto', 'ancora' => 'Centro, Jardim Irajá e o Leite Lopes'],
        ['tipo' => 'cidade', 'estado' => 'sao-paulo', 'cidade' => 'campinas', 'ancora' => 'Cambuí, Unicamp e o Viracopos'],
    ],
];
